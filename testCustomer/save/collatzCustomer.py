...
class TaskDistributor:
    ...
    def initTasks (self):
        for taskNo in range (0, self.number_tasks):
            ...
            taskInfo = { "project": project_name, 
                         "program": {"id": taskBlobID, "size": collatz_fs},
    ...
    def processResults(self, bytedata):
        ...
        for i in range(left, right):
            ...
            seqlength = 0
            for j in range(32, 32 + interval * 4):
                seqlength += bytedata[j]<<8*(j % 4) # oh dear
            print((i, seqlength))
            ...
            for blobid, metadata  in metaDict.items():
                ...
                if (bool(metadata)):
                    if (metadata["result"] == True):
                        processResults(metadata["control"])
                        dataRecieved += 1
                        #### Can delete here ####
...
if __name__ == '__main__':
    ...
    distributor = TaskDistributor(token, 5)
